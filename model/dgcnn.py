...
def get_graph_feature(data, feature_name, k=2):
    # xyz = data[:, :3, :]
    xyz = data[:, :, :]
...
class DGCNN(nn.Module):
    def __init__(self, features, neighboursnum=3, emb_dims=2048):
        ...
        self.conv1 = nn.Conv2d(171, 256, kernel_size=1, bias=False)
        self.conv2 = nn.Conv2d(256, 256, kernel_size=1, bias=False)
        self.conv3 = nn.Conv2d(256, 512, kernel_size=1, bias=False)
        self.conv4 = nn.Conv2d(512, 1024, kernel_size=1, bias=False)
        self.conv5 = nn.Conv2d(2048, emb_dims, kernel_size=1, bias=False)
...
class DGCNN_Nodege(nn.Module):
    def __init__(self, features, neighboursnum=3, emb_dims=1024):
        super(DGCNN_Nodege, self).__init__()
        ...
        # self.conv5 = nn.Conv2d(1024, emb_dims, kernel_size=1, bias=False)
        self.bn1 = nn.BatchNorm2d(256)
        self.bn2 = nn.BatchNorm2d(256)
