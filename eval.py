...
parser.add_argument(
    ...
    default='/data/hzb/project/Graph_matching/GyralHinge_dataset/ground_truth/GT_allpoint_0118_delete/GT_train/',
...
parser.add_argument(
    '--feature_path', type=str, default='/data/hzb/project/Graph_matching/GyralHinge_dataset/feat_node_edge_all_point_with_aparc/',
...
class HyperGHMatching(nn.Module):
    ...
    def __init__(self):
        ...
        self.pointfeaturer1 = DGCNN(cfg.PGM.FEATURES, 1, cfg.PGM.FEATURE_EDGE_CHANNEL)
        ...
        for i in range(self.matching_layer):
            if i == 0:
                self_hyperedge_layer = Siamese_Econv(2048, cfg.PGM.GNN_FEAT)
            else:
                self_hyperedge_layer = Siamese_Econv(cfg.PGM.GNN_FEAT, cfg.PGM.GNN_FEAT)
            self.add_module('self_enn_layer_{}'.format(i), self_hyperedge_layer)

            self.add_module('affinity_s1_{}'.format(i), Affinity(cfg.PGM.GNN_FEAT))
            self.add_module('affinity_s2_{}'.format(i), Affinity(cfg.PGM.GNN_FEAT))
            self.add_module('affinity_s3_{}'.format(i), Affinity(cfg.PGM.GNN_FEAT))
            ...
            self.add_module('hyperedge_to_node_layer_{}'.format(i), Hyperedge_to_node_conv(cfg.PGM.GNN_FEAT, cfg.PGM.GNN_FEAT))
    ...
    def forward(self, data):
        ...
        src_brainarea = torch.tensor(src_brainarea, dtype=torch.float32).to(device)
        tgt_brainarea = torch.tensor(tgt_brainarea, dtype=torch.float32).to(device)
        ...
        for i in range(self.matching_layer):
            ...
            emb_edge_src, emb_edge_tgt = self_enn_layer([torch.bmm(src_w,hyper_src.permute(0, 2, 1).contiguous()), emb_src], [torch.bmm(tgt_w,hyper_tgt.permute(0, 2, 1).contiguous()), emb_tgt])
            ...
            s_s2 = affinity_s2(emb_edge_src[:,5:(hyper_src.shape[2]- hyper_src.shape[1]),:], emb_edge_tgt[:,5:(hyper_tgt.shape[2]- hyper_tgt.shape[1]),:])
            s_s3 = affinity_s3(emb_edge_src[:,(hyper_src.shape[2]- hyper_src.shape[1]):emb_edge_src.shape[1],:], emb_edge_tgt[:,(hyper_tgt.shape[2]- hyper_tgt.shape[1]):emb_edge_tgt.shape[1],:])
            ...
            log_s_s1 = sinkhorn_rpm(s_s1, n_iters=20, slack=cfg.PGM.SKADDCR)
            log_s_s2 = sinkhorn_rpm(s_s2, n_iters=20, slack=cfg.PGM.SKADDCR)
            log_s_s3 = sinkhorn_rpm(s_s3, n_iters=20, slack=cfg.PGM.SKADDCR)
            ...
            s[0, (hyper_src.shape[2]- hyper_src.shape[1]):emb_edge_src.shape[1], (hyper_tgt.shape[2]- hyper_tgt.shape[1]):emb_edge_tgt.shape[1]] = s_s3
        ...
        gt_3 = gt[0, (hyper_src.shape[2]- hyper_src.shape[1]):emb_edge_src.shape[1], (hyper_tgt.shape[2]- hyper_tgt.shape[1]):emb_edge_tgt.shape[1]]
        ...
        for i in range(0,5):
            ...
            loss_s1_s2_s = loss_s1_s2_s + torch.sum(torch.square(emb_edge_src[:,i,:] - torch.sum(emb_edge_src[0, t3 + 5, :],dim=0)))

        loss_s2_s3_s = 0
        for i in range(5,(hyper_src.shape[2]- hyper_src.shape[1])):
            t1 = torch.where(hyper_src[0,:,i] == 1)[0]
            loss_s2_s3_s = loss_s2_s3_s + torch.sum(torch.square(emb_edge_src[:,i,:] - torch.sum(emb_edge_src[0, t1 + (hyper_src.shape[2]- hyper_src.shape[1]), :],dim=0)))
        ...
        for i in range(0,5):
            ...
            t3 = torch.unique(t2[1])
            loss_s1_s2_t = loss_s1_s2_t +torch.sum(torch.square(emb_edge_tgt[:,i,:] - torch.sum( emb_edge_tgt[0, t3 + 5, :],dim=0)))


        loss_s2_s3_t = 0
        for i in range(5,(hyper_src.shape[2]- hyper_src.shape[1])):
            t1 = torch.where(hyper_tgt[0,:,i] == 1)[0]
            loss_s2_s3_t = loss_s2_s3_t + torch.sum(torch.square(emb_edge_tgt[:,i,:] - torch.sum( emb_edge_tgt[0, t1 + (hyper_tgt.shape[2]- hyper_tgt.shape[1]), :],dim=0)))
        ...
        tmp2 = gt[0,(hyper_src.shape[2]- hyper_src.shape[1]):emb_edge_src.shape[1], (hyper_tgt.shape[2]- hyper_tgt.shape[1]):emb_edge_tgt.shape[1]] + scores2[0,(hyper_src.shape[2]- hyper_src.shape[1]):emb_edge_src.shape[1], (hyper_tgt.shape[2]- hyper_tgt.shape[1]):emb_edge_tgt.shape[1]]
        acc2 = torch.where(tmp2 == 2)[0].shape[0] / torch.where(gt[0,(hyper_src.shape[2]- hyper_src.shape[1]):emb_edge_src.shape[1], (hyper_tgt.shape[2]- hyper_tgt.shape[1]):emb_edge_tgt.shape[1]] == 1)[0].shape[0]
...
if __name__ == '__main__':
    with torch.cuda.device(0):
        ...
        # feature_edge = io.loadmat(opt.train_path + '/100206.L.feat_node_edge.mat')

        opt.GT_train_path = opt.GT_eval_path
        ...
        for epoch in range(1, opt.epoch + 1):
            for qq in range(1, 40000 + 1):
                ...
                acc = float()
                loss = float()
                model = "model_epoch_{}_i_{}_our_0302_AB_1_0001.pth".format(epoch, qq)

                print(model)
                hyperghmatching = torch.load([opt.model_path + model][0], map_location={'cuda:0': 'cuda:0'})
                hyperghmatching.eval()
                ...
                for i, gt0_name in enumerate(train_loader):
                    ...
                    loss += data1['loss']
                    # print(data1['accuracy2'])
                    # print(data1['loss'])

                print(acc / 100)

                acc_fname = "accurate_records_eval_surf_0301_AB_1_0001.txt".format(opt.learning_rate)
                with open(acc_fname, 'a') as f:
                        f.write('%.3f\n' % (acc/100))
